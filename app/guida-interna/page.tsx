...
export default function IndoorGuidePage() {
  ...
  async function openHub(item: Hub) {
    setHub(item); setDestination(null); setPoints([]); setLoading(true); setLevel("Tutti"); setCurrentFloor(null); setFloorSource("unknown"); setFloorReference(null);
  ...
  function confirmFloor(value: string) {
    ...
    setMessage(`Piano ${value} confermato. Userò questo dato per la navigazione${position?.altitude != null ? " e per stimare eventuali cambi di piano" : ""}.`);
  }
  ...
  const routeInfo = useMemo(() => {
    ...
    if (!currentFloor || !destinationFloor || currentFloor === destinationFloor) {
      return { route: [position, destination], text: currentFloor && destinationFloor ? `Percorso sul piano ${destinationFloor}` : "Percorso indicativo sulla mappa" };
    }
  ...
  return <main className="indoor-page">
    ...
    <footer className="indoor-source">Mappa e dati: © OpenStreetMap contributors. Il piano automatico è una stima basata sulla variazione di quota dopo una conferma iniziale; se il telefono non fornisce quota affidabile, Varga Tour richiede la conferma manuale.</footer>
  </main>;
}
